...
"""A Code Intelligence Language Engine for the Mako language.
...
Module Usage:
    from cile_mako import scan
    mtime = os.stat("bar.mako")[stat.ST_MTIME]
    content = open("bar.mako", "r").read()
    scan(content, "bar.mako", mtime=mtime)
"""
...
log = logging.getLogger("cile.mako")
...
def scan_buf(buf, mtime=None, lang="Mako"):
    """Scan the given MakoBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {MakoBuffer} is the Mako buffer to scan
    ...
    # Dev Note:
    # This is where you process the Mako content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-Mako-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for Mako content, you may
    #   want to use it.
